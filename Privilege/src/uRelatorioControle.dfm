object frmRelatorioControle: TfrmRelatorioControle
  Left = 294
  Top = 278
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Controle de Cart'#245'es'
  ClientHeight = 298
  ClientWidth = 520
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poOwnerFormCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object gbIntervalo: TGroupBox
    Left = 208
    Top = 0
    Width = 167
    Height = 73
    Caption = 'Intervalo de Datas'
    TabOrder = 0
    object Inicial: TLabel
      Left = 24
      Top = 21
      Width = 27
      Height = 13
      Caption = 'Inicial'
    end
    object Label1: TLabel
      Left = 29
      Top = 47
      Width = 22
      Height = 13
      Caption = 'Final'
    end
    object dtFinal: TDateTimePicker
      Left = 57
      Top = 41
      Width = 86
      Height = 24
      Date = 40802.426179826390000000
      Time = 40802.426179826390000000
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object dtInicial: TDateTimePicker
      Left = 57
      Top = 15
      Width = 86
      Height = 24
      Date = 40802.426179826390000000
      Time = 40802.426179826390000000
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
    end
  end
  object gbFiltros: TGroupBox
    Left = 0
    Top = 0
    Width = 207
    Height = 73
    Caption = 'Relat'#243'rio'
    TabOrder = 1
    object Bevel1: TBevel
      Left = 104
      Top = 13
      Width = 2
      Height = 52
      Shape = bsRightLine
    end
    object rbEmitidos: TRadioButton
      Left = 8
      Top = 16
      Width = 58
      Height = 17
      Caption = 'Emitidos'
      Checked = True
      TabOrder = 0
      TabStop = True
      OnMouseUp = rbEmitidosMouseUp
    end
    object rbBloqueados: TRadioButton
      Left = 112
      Top = 43
      Width = 77
      Height = 17
      Caption = 'Bloqueados'
      TabOrder = 1
      OnMouseUp = rbBloqueadosMouseUp
    end
    object rbMovimentados: TRadioButton
      Left = 8
      Top = 43
      Width = 89
      Height = 17
      Caption = 'Movimentados'
      TabOrder = 2
      OnMouseUp = rbMovimentadosMouseUp
    end
    object rbSolicitados: TRadioButton
      Left = 112
      Top = 16
      Width = 73
      Height = 17
      Caption = 'Solicitados'
      TabOrder = 3
      OnMouseUp = rbSolicitadosMouseUp
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 74
    Width = 520
    Height = 223
    TabOrder = 2
    object dbControle: TDBGrid
      Left = 4
      Top = 8
      Width = 512
      Height = 197
      DataSource = dsGrid
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
    end
  end
  object GroupBox1: TGroupBox
    Left = 376
    Top = 0
    Width = 143
    Height = 73
    TabOrder = 3
    object btExcel: TBitBtn
      Left = 32
      Top = 42
      Width = 75
      Height = 25
      Caption = 'Excel'
      Glyph.Data = {
        
        92805F93813D7560528C703C7C593C7D563E86582F784635804C}
      TabOrder = 0
      OnClick = btExcelClick
    end
    object btMostrar: TBitBtn
      Left = 32
      Top = 14
      Width = 75
      Height = 25
      Caption = 'Mostrar'
      Glyph.Data = {
        
        3333333333333333333333333333333333333333333333333333}
      NumGlyphs = 2
      TabOrder = 1
      OnClick = btMostrarClick
    end
  end
  object StatusBar: TStatusBar
    Left = 0
    Top = 279
    Width = 520
    Height = 19
    Panels = <
      item
        Width = 400
      end
      item
        Alignment = taCenter
        Width = 50
      end>
  end
  object qryRelControlePriv: TIBQuery
    Database = dmDescontos.bdDescontos
    Transaction = dmDescontos.IBTransaction2
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    Left = 152
    Top = 185
  end
  object dsGrid: TDataSource
    DataSet = qryRelControlePriv
    Left = 240
    Top = 177
  end
end
